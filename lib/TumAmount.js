...
Amount.prototype.parse_issuer = function(issuer) {
  if (base_wallet.isValidAddress(issuer)) this._issuer = issuer
...
Amount.prototype.parse_json = function(in_json) {
  if (typeof in_json === "number") {
    ...
    if (!isTumCode(in_json.currency))
      ...
      } else this.parse_swt_value(in_json.value.toString())
    }
  } else throw new Error("Amount.parse_json: Unsupported JSON type!")
  return this
}
...
Amount.prototype.to_json = function() {
  ...
  if (this._is_native) {
    ...
    if (this._issuer.is_valid()) amount_json.issuer = this._issuer
    result = amount_json
  }
