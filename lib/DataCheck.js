...
function isCurrency(in_code) {
  return typeof in_code === "string"
    ? in_code &&
      in_code.length >= CURRENCY_NAME_LEN &&
      in_code.length <= CURRENCY_NAME_LEN2
      ? true
      : false
    : false
}
...
function isCustomTum(in_code) {
  if (isLetterNumer(in_code) && String(in_code).length === TUM_NAME_LEN) {
    return true
  } else return false
}
...
function equals(that) {
  return (
    that != null && //input must be defined and non-null
    that.constructor === Tum && //and an instance of Tum
    this.code === that.code &&
    this.issuer === that.issuer
  ) //have the same code and issuer
}
...
function isTumCode(in_code) {
  return (
    typeof in_code == "string" && //input must be defined and non-null
...
function isRelation(in_str) {
  if (
    typeof in_code == "string" &&
    (in_str == "freeze" || in_str == "autorize")
  )
    return true
  else return false
}
...
function isAmount(in_obj) {
  if (typeof in_obj != "object") return false
  if (typeof in_obj.value !== "string" || !isFloat(in_obj.value)) return false
  if (!isTumCode(in_obj.currency)) return false
  //AMOUNT could have a field named
  //either as 'issuer'
  //or as 'counterparty'
  //for SWT, this can be undefined
  if (typeof in_obj.issuer != "undefined" && in_obj.issuer != "") {
    if (!base.isValidAddress(in_obj.issuer)) return false
  } else {
...
function buildAmount(in_currency, in_issuer, in_value) {
  ...
  if (!isFloat(in_value)) throw new ParamException("Invalid value")
...
function isBalance(in_obj) {
  ...
  if (!isFloat(in_obj.freezed)) return false

  if (!isFloat(in_obj.value)) return false
  ...
  if (!base.isValidAddress(in_obj.counterparty)) return false
