...
Seed.prototype.get_key = function(account, maxLoops) {
  ...
  var compressed = [header].concat(
    sjcl.codec.bytes.fromBits(public_gen.x.toBits())
  )
  ...
  do {
    ...
    do {
      ...
      sec = sjcl.bn.fromBits(
        firstHalfOfSHA512(append_int(append_int(compressed, account_number), i))
      )
      i++
    ...
    if (max_loops-- <= 0) {
      ...
      throw new Error(
        "Too many loops looking for KeyPair yielding " +
          address.to_json() +
          " from " +
          this.to_json()
      )
